...
gApp.SearchPage.prototype = {
	...
	setUp:function()
	{
		...
		//this.setUpCostFilter(); IN PROGRESS FOR CREATION OF FILTER
		this.setUpRangeFilter()
	...
	setUpPopUpListeners:function()
	{
		...
		window.on("popupBackground","click",function()
		{
				document.getElementById("popUp").style.display ="none";
		})
	},
	setUpRangeFilter :function()
	{
		window.on("applyCostFilter","click",this.applyRangeFilterHandler.bind(this,"first"));
		
		
	},
	resetFilter :function()
	{
		var i , domEle = document.getElementById("search_result_content").childNodes;
		for(i =0;i<domEle.length;i++)
		{
			domEle[i].style.display="block"
		}
	},
	
	applyFilter :function(start,end)
	{
		var i , domEle = document.getElementById("search_result_content").childNodes;
		for(i =0;i<domEle.length;i++)
		{
			domEle[0].style.display="none"
			if(Number(domEle[0].getAttribute("singlerowcost")) >= start && Number(domEle[0].getAttribute("singlerowcost")) <= end)
				domEle[0].style.display="block"
		}
	},
	applyRangeFilterHandler :function()
	{
		var startCost = document.getElementById("startCost").value;
		var endCost = document.getElementById("startCost").value;
		
		if(document.getElementById("applyCostFilter").innerHTML === "Reset")
		{
			this.resetFilter();
			document.getElementById("applyCostFilter").innerHTML = "Apply Filter"
			return false;
		}
		
		if(!this.isValidCount(startCost))
		{
			this.indicateInvalidAction(gApp.AppConstant.INVALID_START_RANGE);	
			return false;
		}
		
		
		if(!this.isValidCount(endCost))
		{
			this.indicateInvalidAction(gApp.AppConstant.INVALID_END_RANGE);	
			return false;
		}
		
		document.getElementById("applyCostFilter").innerHTML = "Reset"
		//apply filter
		
		this.applyFilter(startCost,endCost);
	...
	setUpCostFilter :function()
	{
			...
			window.on("idMarkerFirst","mousedown",this.markerKeyDown.bind(this,"first"));
			window.on("idMarkerFirst","mousemove",this.markerMouseMove.bind(this,"first"));
			window.on("idMarkerFirst","mouseup",this.markerMouseUp.bind(this,"first"));
			
			//window.on("idMarkerLast","mousedown",this.markerKeyDown.bind(this,"last"));
			this.leftMouseMoveEnabled =false;
			this.leftStateX = window.getPosition(document.getElementById("idMarkerFirst"));	
			this.leftValueStateX = 0;
				
		
	},
	markerKeyDown:function(data)
	{
		this.leftMouseMoveEnabled =true;
		this.leftStateX = window.getPosition(document.getElementById("idMarkerFirst"));
		console.log("Mouse Move START");
	},
	markerMouseMove:function(data)
	{
		console.log(" move..");
		if(this.leftMouseMoveEnabled)
		{
			
			var left = 0
			console.log("Mouse Move Move"+event.pageX);
				if(this.leftStateX.x < event.pageX)
					{
						left = event.pageX -  this.leftStateX.x;
						if(left > 3)
						{
							document.getElementById("idMarkerFirst").style.marginLeft =  this.leftValueStateX+"%";
							this.leftValueStateX++;
						}
					}
		}
	},
	markerMouseUp:function(data)
	{
		this.leftMouseMoveEnabled =false;
		console.log("Mouse Move STOP");
	},
	...
	*/
	...
	{
		var i, arr = [] ,sT="",domEle,domParent = document.getElementById("search_result_content"), startRange=0, endRange=0;
		domParent.innerHTML = "";
		...
		for(i =0;i< arrData.length;i++)
		{
			...
			// WAY TO CHECK RETURN IF SENT FROM SERVER
			arrData[i]["displayReturn"] = (gApp.AppConstant.SEARCH_RESULT_METADATA["endDate"] === "")?"none":"block";
			if(arrData[i]["displayReturn"] === "block")
				arrData[i]["displayReturn"] = (arrData[i]["isReturn"] === true)?"block":"none";
			...
			if(i == 0)
			{
				startRange = Number(arrData[i]["cost"]);
				endRange = Number(arrData[i]["cost"]) + 1;
			}
			if(arrData[i]["cost"] < startRange)
				startRange = Number(arrData[i]["cost"])
				
			if(arrData[i]["cost"] > endRange)
				endRange = Number(arrData[i]["cost"]);	
		}
		
		document.getElementById("startCost").value = startRange;
		document.getElementById("endCost").value = endRange;
		 
	},
	bookBtnHandler:function(evt)
	{
		this.indicateInvalidAction("BOOK BUTTON CLICKED");
	},
	...
	*/	
	...
	{
		document.getElementById("popupTextContent").innerHTML = cmd;
